...
class KnobScripter(QtWidgets.QWidget):
    ...
    def createPrefsMenu(self):
        ...
        # Actions
        self.echoAct = QtWidgets.QAction("Echo python commands", self, checkable=True, statusTip="Toggle nuke's 'Echo all python commands to ScriptEditor'",
    ...
    def saveKnobValue(self, check=True):
        ...
        if check and obtained_knobValue != edited_knobValue:
            ...
            if reply == QtWidgets.QMessageBox.No:
                return
        # ADD UNICODE SUPPORT TO KNOBS
        try:
            self.node[dropdown_value].setValue(edited_knobValue)
        except UnicodeEncodeError:
            self.node[dropdown_value].setValue(edited_knobValue.encode('utf-8'))
    ...
    def toggleFRW(self, frw_pressed):
        ...
        if self.frw_open:
            # SELECT ON FIND
            cursor = self.script_editor.textCursor()
            find_str = cursor.selectedText() if cursor.hasSelection() else ""
            self.frw.find_lineEdit.setText(find_str)
...
class KSScriptEditorHighlighter(QtGui.QSyntaxHighlighter):
    ...
    def __init__(self, document):
        ...
        self.keywords = [
            'as', 'and', 'assert', 'break', 'class', 'continue', 'def',
...
class KnobScripterTextEditMain(KnobScripterTextEdit):
    ...
    def keyPressEvent(self, event):
        ...
        if not self.nukeCompleterShowing and (ctrl or shift or alt):
            # Bypassed!
            # ADD ALT+BACKSPACE SUPPORT TO CLEAR OUTPUT
            if key not in [Qt.Key_Return, Qt.Key_Enter, Qt.Key_Tab, Qt.Key_Backspace]:
        ...
        if type(event) == QtGui.QKeyEvent:
            if key == Qt.Key_Escape:  # Close the knobscripter...
                ...
                try:  # Meaning snippet found
                    ...
                    KnobScripterTextEdit.keyPressEvent(self, event)
            # ADD ALT+BACKSPACE SUPPORT TO CLEAR OUTPUT
            elif alt and event.key() == Qt.Key_Backspace:
                self.script_output.setPlainText("")
        ...
        if not allModules:
            if len(moduleSearchString.split('.')) == 1:
                ...
                try:
                    ...
                    matchedModules = [x for x in dir(getattr(sys.modules[trimmedModuleSearchString], moduleSearchString.split('.')[-1])) if
...
class SnippetEdit(QtWidgets.QWidget):
    ...
    def __init__(self, key="", val=""):
        ...
        self.script_editor_font.setStyleHint(QtGui.QFont.Monospace)
        self.script_editor_font.setFixedPitch(True)
        self.script_editor_font.setPointSize(10)
